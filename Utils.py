...
###############################################################################
def IMAGXMAPS(image,maps,name='IMAGXMAP'):
	with tf.variable_scope(name+'_scope'):
		image = tf_complex(image)
		maps = tf_complex(maps)
		TxM = tf.abs(image*maps)
		TxM = tf_channel(TxM)
	return tf.identity(TxM,name=name)


def MULTICOILRF(image,mask,name='MULTICOILRF'):
	with tf.variable_scope(name+'_scope'):
		for coil in range(0,8):
			tmp = RF(tf.expand_dims(image[coil,:,:,:], axis=0),mask)
			if (coil==0):
				MULTI_frq = tmp
			else:
				MULTI_frq = tf.concat([MULTI_frq,tmp], 0) 
	return tf.identity(MULTI_frq,name=name)
###############################################################################
...
class ImageDataFlow(RNGDataFlow):
	def __init__(self, imageDir, maskDir, mapDir, labelDir, size, ratio = 0.1, dtype='float32', is_training=False):
		"""
		Args:
	...
	def get_data(self, shuffle=True):
		# self.reset_state()
		images = glob.glob(self.imageDir + '/imgGT_*.npy')
		#print "images: ", images
		if self.maskDir:
			masks  = glob.glob(self.maskDir + '/*.tif')
		# print "masks: ", masks
		labels = glob.glob(self.labelDir + '/imgGT_*.npy')
		# print "labels: ", labels
		maps = glob.glob(self.mapDir+'/SensitivityMaps_*.npy')
		...
		for k in range(self._size):
			...
			image = np.load(images[rand_index_image])
			#### add by jun for transbrain
			#if DIMX == 256:
			#	image = np.pad(image,((0,0),(19,19)),'constant')
			image = image/image.max()
			
			if self.maskDir:
				mask  = cv2.imread(masks[rand_index_mask],0)
				#mask = np.resize(mask,[512,512])
				mask = (mask/255-0.5)*2.0
#				mask = mask/255
			else:
				mask = 255*self.generateMask(DIMZ, DIMY, DIMX, sampling_rate=self.ratio)
			
			label = np.load(labels[rand_index_label])
			#if DIMX == 256:
			#	label = np.pad(label,((0,0),(19,19)),'constant')
			...
			image_map = np.load(maps[rand_index_image])
			image_map_combine = abs(image_map[:,:,:,1]+1j*image_map[:,:,:,1])
			image_map_combine = image_map_combine/image_map_combine.max()
			del image_map
			image_map = np.stack((image_map_combine,np.zeros_like(image_map_combine)), axis=0)
			image_map = image_map.transpose(3,0,1,2)
	

			label_map = np.load(maps[rand_index_label])
			label_map_combine = abs(label_map[:,:,:,1]+1j*label_map[:,:,:,1])
			label_map_combine = label_map_combine/label_map_combine.max()
			del label_map
			label_map = np.stack((label_map_combine,np.zeros_like(label_map_combine)), axis=0)
			label_map = label_map.transpose(3,0,1,2)
			...
			yield [image.astype(np.float32), 
				   ...
				   image_map.astype(np.float32),
				   label_map.astype(np.float32)]

def get_data(imageDir, maskDir, mapDir,labelDir, size=EPOCH_SIZE):
