...
from GAN import MultiGPUGANTrainer, GANTrainer, GANModelDesc, SeparateGANTrainer
...
class Model(GANModelDesc):
	def _get_inputs(self):
		return [InputDesc(tf.float32, (None, 2, DIMY, DIMX), 'inputA'),
				...
				InputDesc(tf.float32, (None, 2, DIMY, DIMX), 'mapA'), 
				InputDesc(tf.float32, (None, 2, DIMY, DIMX), 'mapB'),
				]
	...
	def _build_graph(self, inputs):
		...
		with tf.name_scope('losses'):
			with tf.name_scope('Frq'):
				with tf.name_scope('Recon'):
					...
					recon_frq_sense_AA = tf.reduce_mean(tf.abs((RF(S01, R) - MULTICOILRF(IMAGXMAPS(Sp1,mapA), R))), name='recon_frq_sense_AA')
					recon_frq_sense_BB = tf.reduce_mean(tf.abs((RF(S02, R) - MULTICOILRF(IMAGXMAPS(Sp2,mapB), R))), name='recon_frq_sense_BB')
			...
			with tf.name_scope('Img'):
				...
				with tf.name_scope('Recon'):
					...
					recon_sense_AA = tf.reduce_mean(tf.abs(S01-IMAGXMAPS(S1,mapA)),name='recon_sense_AA')
					recon_sense_BB = tf.reduce_mean(tf.abs(S02-IMAGXMAPS(S2,mapB)),name='recon_sense_BB')
		...
		ALPHA = 1e+1
		print('I want to check G_loss_AA:',G_loss_AA)
		GAMMA = 1e-0
		DELTA = 1e-4
		...
		self.g_loss = tf.add_n([
								...
								#(G_loss_Aa + G_loss_Bb + G_loss_Ab + G_loss_Ba)*0,
								#(recon_img_AA + recon_img_BB) * 1.00 * ALPHA * RATES, 
								(recon_sense_AA + recon_sense_BB) * 1.00 * ALPHA * RATES, 
								#(recon_img_Aa + recon_img_Bb) * 1.00 * ALPHA * RATES*0, 
								(error_img_AA + error_img_BB) * 1e+2 * ALPHA * RATES, 
								#(error_img_Aa + error_img_Bb) * 1e+2 * ALPHA * RATES*0, 
								#(recon_frq_AA + recon_frq_BB) * 1.00 * GAMMA * RATES, 
								(recon_frq_sense_AA+recon_frq_sense_BB)*1.00 * GAMMA * RATES, 
								#(recon_frq_Aa + recon_frq_Bb) * 1.00 * GAMMA * RATES, 
								#(smoothness_AA + smoothness_BB + smoothness_Aa*0 + smoothness_Bb*0) * DELTA, 
								(smoothness_AA + smoothness_BB ) * DELTA, 
								], name='G_loss_total')
		self.d_loss = tf.add_n([
								(D_loss_AA + D_loss_BB + D_loss_AB + D_loss_BA), 
								#(D_loss_Aa + D_loss_Bb + D_loss_Ab + D_loss_Ba)*0, 
								], name='D_loss_total')
...
#		psnr(tf_complex(M1), tf_complex(S01), maxp=255, name='PSNR_zfill_A')
#		psnr(tf_complex(M2), tf_complex(S02), maxp=255, name='PSNR_zfill_B')
#		psnr(tf_complex(S1), tf_complex(S01), maxp=255, name='PSNR_recon_A')
#		psnr(tf_complex(S2), tf_complex(S02), maxp=255, name='PSNR_recon_B')
#		psnr(tf_complex(T1), tf_complex(S01), maxp=255, name='PSNR_boost_A')
#		psnr(tf_complex(T2), tf_complex(S02), maxp=255, name='PSNR_boost_B')
		psnr(tf_complex(cvt2imag(M1)), tf_complex(cvt2imag(S01)), name='PSNR_zfill_A')
		psnr(tf_complex(cvt2imag(M2)), tf_complex(cvt2imag(S02)), name='PSNR_zfill_B')
		psnr(tf_complex(cvt2imag(S1)), tf_complex(cvt2imag(S01)), name='PSNR_recon_A')
		psnr(tf_complex(cvt2imag(S2)), tf_complex(cvt2imag(S02)), name='PSNR_recon_B')
		#psnr(tf_complex(cvt2imag(T1)), tf_complex(cvt2imag(S01)), name='PSNR_boost_A')
		#psnr(tf_complex(cvt2imag(T2)), tf_complex(cvt2imag(S02)), name='PSNR_boost_B')
...
#		psnr(tf_complex(cvt2imag(T2)), tf_complex(cvt2imag(S02)), maxp=255, name='PSNR_boost_B')
		...
		def viz3(name, listTensor):
			...
			#print('viz33333333333333333333333')
			#print(img.shape)
			out = img
			img = cvt2imag(img)
		...
		viz_A_recon, vis_A_recon = viz3('A_recon', [R, S01, M1, S1])# tf.abs(S01-M1), tf.abs(S01-S1), Sn1, Sp1])
		viz_B_recon, vis_B_recon = viz3('B_recon', [R, S02, M2, S2])# tf.abs(S02-M2), tf.abs(S02-S2), Sn2, Sp2])
...
###############################################################################	
def sample(imageDir, maskDir, mapDir,labelDir, model_path, resultDir):
	# TODO
	print (sys.argv[0])
	pred_config = PredictConfig(
		session_init=SaverRestore(model_path), #session_init=SaverRestore(args.load)
		model=Model(),
		input_names=['inputA', 'mask', 'inputB','mapA','mapB'],
		output_names=['vis_A_recon'])


	ds_valid = ImageDataFlow(imageDir, maskDir, mapDir, labelDir, 400, is_training=False)
	# ds_valid = PrefetchDataZMQ(ds_valid, nr_proc=8)
	
	filenames = glob.glob(imageDir + '/imgGT_*.npy')
...
#		print ('I AM HERE')
		...
		#np.save(resultDir+ "/full_"+tail, np.squeeze(colors0[...,512*1:512*2])) # Zerofill
		#np.save(resultDir+"/zfill_"+tail, np.squeeze(colors0[...,512*2:512*3])) # Zerofill
		np.save(resultDir+'/GAN_'+tail, np.squeeze(colors0)) # Zerofill
		#np.save(resultDir+tail, np.squeeze(colors0[...,512*4:512*5])) # Zerofill

		#np.save(resultDir+"mag/mag_"+tail, np.abs(np_complex(np.squeeze(colors0[...,512*4:512*5]))))
		#np.save(resultDir+"ang/ang_"+tail, np.angle(np_complex(np.squeeze(colors0[...,512*4:512*5]))))


		#np.save(resultDir+"/M/mag/mag_"+tail, np.abs(np_complex(np.squeeze(colors0[...,512*2:512*3]))))
		#np.save(resultDir+"/M/ang/ang_"+tail, np.angle(np_complex(np.squeeze(colors0[...,512*2:512*3]))))
...
class VisualizeRunner(Callback):
	...
	def _before_train(self):
		global args
		self.ds_train, self.ds_valid = get_data(args.imageDir, args.maskDir, args.mapDir, args.labelDir, size=1)
...
def main():
	...
	#
	parser.add_argument('--gpu',   default='1,2',      help='comma separated list of GPU(s) to use.')
	...
	parser.add_argument('--imageDir',default='data/brain/db_train/',   help='Image directory', required=True)
	parser.add_argument('--maskDir', default='data/mask/radial/mask_1/',   help='Masks directory', required=False)
	parser.add_argument('--mapDir',default='data/brain/db_train/',   help='Map directory', required=True)
	parser.add_argument('--labelDir',default='data/brain/db_train/',   help='Label directory', required=True)
	...
	if args.sample:
		sample(args.imageDir, args.maskDir, args.mapDir,args.labelDir, args.load, args.sample)
	else:
		logger.auto_set_dir()
		ds_train, ds_valid = get_data(args.imageDir, args.maskDir, args.mapDir, args.labelDir)
		...
		trainer.train_with_defaults(
			callbacks=[
				...
				PeriodicTrigger(MaxSaver('validation_PSNR_recon_A'), every_k_epochs=100),
				#(MaxSaver('validation_PSNR_boost_A'), every_k_epochs=20),
				VisualizeRunner(),
				InferenceRunner(ds_valid, [
										   ...
										   ScalarStats('losses/Frq/Recon/recon_frq_sense_AA'),
										   ScalarStats('losses/Frq/Recon/recon_frq_sense_BB'),

										   #ScalarStats('losses/Img/Recon/recon_img_AA'),
										   #ScalarStats('losses/Img/Recon/recon_img_BB'),
										   ScalarStats('losses/Img/Recon/recon_sense_AA'),
										   ScalarStats('losses/Img/Recon/recon_sense_BB'),
				...
				ScheduledHyperParamSetter('learning_rate', 
					[(0, 1e-4), (200, 3e-4), (400, 2e-5), (600, 1e-5), (800, 2e-6), (1000, 1e-6)], interp='linear')
